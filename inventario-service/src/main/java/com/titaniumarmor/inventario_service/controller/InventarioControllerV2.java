...
import com.titaniumarmor.inventario_service.assembler.InventarioModelAssembler;
...
public class InventarioControllerV2 {
    ...
    public CollectionModel<EntityModel<Inventario>> listarInventarios() {
        logger.info("V2 GET /inventarios - Listando inventarios");
        List<EntityModel<Inventario>> inventarios = inventarioService.listar().stream()
                .map(assembler::toModel)
                .collect(Collectors.toList());
        return CollectionModel.of(inventarios, linkTo(methodOn(InventarioControllerV2.class).listarInventarios()).withSelfRel());
    }

    @GetMapping("/{id}")
    public EntityModel<Inventario> obtenerInventario(@PathVariable Long id) {
        logger.info("V2 GET /inventarios/{} - Obteniendo inventarios", id);
